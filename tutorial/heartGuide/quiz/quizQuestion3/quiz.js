...
let score = parseFloat(window.localStorage.getItem("score"));
...
let overlayResult = document.querySelector(".overlayResult");
...
console.log("hola");
        ...
        parseInt(cursor.style.top.split("px")) > resultPopUp.offsetTop &&
        parseInt(cursor.style.top.split("px")) <
          resultPopUp.offsetTop + resultPopUp.offsetHeight &&
        parseInt(cursor.style.left.split("px")) > resultPopUp.offsetLeft &&
        parseInt(cursor.style.left.split("px")) <
          resultPopUp.offsetLeft + resultPopUp.offsetWidth
      ) {
        loader.style.display = "block";
...
function showAnswer(answer) {
  ...
  answerCorrected = true
    switch (answer) {
      case 1:
        optionA.style.filter = ("hue-rotate(250deg) saturate(6)")
        
        break;
      case 2:
        optionA.style.filter = ("hue-rotate(250deg) saturate(6)")
        optionB.style.filter = ("hue-rotate(120deg) saturate(4)")
        break;
      case 3:
...
resultTitle.innerHTML = "Tu puntaje: " + Math.round(score)+'.0'
